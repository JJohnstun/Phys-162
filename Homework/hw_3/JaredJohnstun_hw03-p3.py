...
x, y, z = np.loadtxt('hw03-data.txt', skiprows=3, usecols={0, 1, 3}, unpack=True)
...
y_line = str(f'\nsum of array y is {y_sum: .2f} average = {y_average: .3f}')

z_sum = np.sum(z)
z_average = np.average(z)
z_line = str(f'\nsum of array z is {z_sum: .2f} average = {z_average: .3f}')
